...
def main():
    ...
    default_root_dir = "./logs/44k/"
    speaker = "jiaran"
    model_root_dir = f"./models/44k_{speaker}/"
    model_map = {
        "jiaran": "G_72000.pth",
        "beilanailin": "G_65600.pth",
        "sinuo": "G_74400.pth",
        "xinyi": "G_92400.pth"
    }
    wav_root_dir = "F:/videos/vocals/"
    wav_list = {
        "jiaran": [
            os.path.join(wav_root_dir, "4_1-【ChiliChill】饿魔少女（emo_girl）【MV】-480P_标清-AVC_(Vocals).wav"),
            # os.path.join(wav_root_dir, "9_1-【Days作品】夏日的梦-‘洛天依’-480P_标清-AVC_(Vocals).wav"),
            # os.path.join(wav_root_dir, "14_1-【洛天依原创曲】二人三角【多图流PV付】-480P_标清-AVC_(Vocals).wav"),
            # os.path.join(wav_root_dir, "8_1-【Days】透明的幻境（重制版）-洛天依-480P_标清-AVC_(Vocals).wav"),
            # os.path.join(wav_root_dir, "12_1-【洛天依原创】心碎烧酒-480P_标清-AVC_(Vocals).wav"),
            # os.path.join(wav_root_dir, "13_1-【洛天依原创】一半一半-480P_标清-AVC_(Vocals).wav"),
            # os.path.join(wav_root_dir, "44_336-【洛天依】《东京不太热》（2024重调版）_“一盏一盏街灯烘焙了夜”【原创PV付】-480P_标清-AVC_(Vocals).wav")
        ],
        "beila": [
            os.path.join(wav_root_dir, "45_338-【洛天依x乐正绫】《三月雨2024》_“空余忆，良辰美景多可惜”【南北组】【原创PV付】-480P_标清-AVC_(Vocals).wav"),
            os.path.join(wav_root_dir, "42_221-【洛天依x乐正绫】《霜雪千年》_“梨花香，缠着衣角掠过熙攘”【2024Remix.ver】【原创PV付】-480P_标清-AVC_(Vocals).wav"),
            os.path.join(wav_root_dir, "16_1-【星尘Infinity】入秋的第一场雨真让人矫情【Synth_V_COVER】【67P】-480P_标清-AVC_(Vocals).wav"),
            os.path.join(wav_root_dir, "10_1-【洛天依AI】入秋的第一场雨真让人矫情（ChiliChill）-480P_标清-AVC_(Vocals).wav")
        ],
        "nailin": [
            os.path.join(wav_root_dir, "45_338-【洛天依x乐正绫】《三月雨2024》_“空余忆，良辰美景多可惜”【南北组】【原创PV付】-480P_标清-AVC_(Vocals).wav"),
            os.path.join(wav_root_dir, "42_221-【洛天依x乐正绫】《霜雪千年》_“梨花香，缠着衣角掠过熙攘”【2024Remix.ver】【原创PV付】-480P_标清-AVC_(Vocals).wav"),
            os.path.join(wav_root_dir, "38_8-【洛天依】橙子汽水（ace_cover）-480P_标清-AVC_(Vocals).wav"),
            os.path.join(wav_root_dir, "35_5-【ChiliChill】橙子汽水_Studio_Live-480P_标清-AVC_(Vocals).wav"),
            os.path.join(wav_root_dir, "20_1-今天不卷了，屑屑-480P_标清-AVC_(Vocals).wav"),
            os.path.join(wav_root_dir, "37_6-「洛天依.翻调」《芭蕉夜雨》｜雨季.芭蕉与三更的月-480P_标清-AVC_(Vocals).wav"),
        ]
    }
    # 一定要设置的部分
    parser.add_argument('-m', '--model_path', type=str, default=os.path.join(model_root_dir, model_map[speaker]), help='模型路径')
    parser.add_argument('-c', '--config_path', type=str, default=os.path.join(model_root_dir, "config.json"), help='配置文件路径')
    parser.add_argument('-cl', '--clip', type=float, default=0, help='音频强制切片，默认0为自动切片，单位为秒/s')
    parser.add_argument('-n', '--clean_names', type=str, nargs='+', default=wav_list[speaker], help='wav文件名列表，放在raw文件夹下')
    parser.add_argument('-t', '--trans', type=int, nargs='+', default=[0], help='音高调整，支持正负（半音）')
    parser.add_argument('-s', '--spk_list', type=str, nargs='+', default=[speaker], help='合成目标说话人名称')
