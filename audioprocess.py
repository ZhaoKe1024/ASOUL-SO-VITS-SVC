...
def rename_clips():
    ...
    for ind, item in enumerate(os.listdir("./dataset_raw/sinuo/")):
        parts = item.split("_")
        new_name = "sinuo_"+parts[1].split('-')[0]+"_"+("000"+str(ind))[-4:]+".wav"
        # print(new_name)
        # break
        update_name_list.append((os.path.join(f"./dataset_raw/sinuo/",item), os.path.join(f"./dataset_raw/sinuo/",new_name)))
    pdb.set_trace()
